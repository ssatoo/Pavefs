...
class  QueryModel :NSObject, ServiceAPICallDelegate
{
   ...
   // var password: String!
    ...
    func RateResponce(rp1:String,rp2:String,rp3:String,rp4:String,queryResultID:String){
    
        let serviceRequest = ServiceMethod.RateResponsesRequest(rp1, rp2: rp2, rp3: rp3, rp4: rp4, queryResultID: queryResultID, token: Settings.getToken(), deviceType: globalConstants.deviceType)
        let serviceCall:ServiceAPI = ServiceAPI()
        serviceCall.apiDelegate = self;
        serviceCall.sendRequestToServer(serviceRequest, operationName:apiMethod.RATERESPONCES_API)
    ...
    func serviceCallDidFinish(response:String, operationName:String)
    {
        ...
        }else if operationName == apiMethod.RATERESPONCES_API{
        
           print("RATERESPONCES_API succes : \(responseDic["success"])")
            delegate.queryService("ssd", moveForward:true)
    ...
    func setUpQuery(queryObj:AnyObject)
    {
         ...
         print("\(queryObj.valueForKey("userTask")!.valueForKey("queryResultId"))")
        ...
        self.query.queryResultId = queryObj.valueForKey("userTask")!.valueForKey("queryResultId") as? String
